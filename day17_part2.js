...
while (currPos.length > 0) {
  ...
  let onward = [-1, -1];
  if (step < 10) {
    onward = [pos[0] + directions[dir][0], pos[1] + directions[dir][1]];
  }
  const right = [
  ...
  if (!isOOB(right)) {
    rightLoss =
      loss +
      gridData[pos[0] + directions[rightTurn[dir]][0]][
        pos[1] + directions[rightTurn[dir]][1]
      ] +
      gridData[pos[0] + 2 * directions[rightTurn[dir]][0]][
        pos[1] + 2 * directions[rightTurn[dir]][1]
      ] +
      gridData[pos[0] + 3 * directions[rightTurn[dir]][0]][
        pos[1] + 3 * directions[rightTurn[dir]][1]
      ] +
      gridData[pos[0] + 4 * directions[rightTurn[dir]][0]][
        pos[1] + 4 * directions[rightTurn[dir]][1]
      ];
  }
  ...
  if (!isOOB(left)) {
    leftLoss =
      loss +
      gridData[pos[0] + directions[leftTurn[dir]][0]][
        pos[1] + directions[leftTurn[dir]][1]
      ] +
      gridData[pos[0] + 2 * directions[leftTurn[dir]][0]][
        pos[1] + 2 * directions[leftTurn[dir]][1]
      ] +
      gridData[pos[0] + 3 * directions[leftTurn[dir]][0]][
        pos[1] + 3 * directions[leftTurn[dir]][1]
      ] +
      gridData[pos[0] + 4 * directions[leftTurn[dir]][0]][
        pos[1] + 4 * directions[leftTurn[dir]][1]
      ];
  }
  ...
  if (!isOOB(onward)) {
    if (loss + gridData[onward[0]][onward[1]] < bestLoss) {
      if (
        !(
          visitData[onward[0]][onward[1]][step - 3][dir] <=
  ...
  if (!isOOB(right)) {
    if (rightLoss < bestLoss) {
      if (!(visitData[right[0]][right[1]][0][rightTurn[dir]] <= rightLoss)) {
        if (right[0] === finalCoord[0] && right[1] === finalCoord[1]) {
          ...
          currPos.push(right);
          currLoss.push(rightLoss);
          for (let x = 0; x < 7; x++) {
            visitData[right[0]][right[1]][x][rightTurn[dir]] = rightLoss;
          }
        }
      }
    }
  ...
  if (!isOOB(left)) {
    if (leftLoss < bestLoss) {
      if (!(visitData[left[0]][left[1]][0][leftTurn[dir]] <= leftLoss)) {
        if (left[0] === finalCoord[0] && left[1] === finalCoord[1]) {
          ...
          currPos.push(left);
          currLoss.push(leftLoss);
          for (let x = 0; x < 7; x++) {
            visitData[left[0]][left[1]][x][leftTurn[dir]] = leftLoss;
          }
        }
      }
    }
