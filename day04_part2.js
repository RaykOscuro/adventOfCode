...
for (let x = 0; x < splitFile.length; x++) {
  let winCount = null;
  let droppedCardInfo = splitFile[x].split(":");
  droppedCardInfo = droppedCardInfo[1].split("|");
  const winningNumbers = droppedCardInfo[0]
    .trim()
    .split(" ")
  ...
  for (let winN of winningNumbers) {
    if (ourNumbers.includes(winN)) {
      winCount === null ? (winCount = 1) : (winCount += 1);
    }
  }
  if (winCount !== null) {
    for (let y = 1; y <= winCount; y++) {
      helperArray[x + y] += helperArray[x];
    }
  }
}

for (let x of helperArray) {
  sumOfEarnings += x;
}
