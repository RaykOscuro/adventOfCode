...
function activateNode(nodeInfo, x) {
  ...
  if (node.type === "&") {
    ...
    if (
      node.name === "ns" &&
      JSON.stringify(node.state[3]) !== JSON.stringify(inputTracker[3])
    ) {
      console.log(x, node.state);
      inputTracker[3] = JSON.parse(JSON.stringify(node.state[3]));
    }
...
for (let x = 1; x <= 1000000; x++) {
  ...
  while (nodeStack.length > 0) {
    const nextNodeInfo = nodeStack.shift();
    activateNode(nextNodeInfo, x);
